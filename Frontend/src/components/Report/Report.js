...
const Report = () => {
  ...
  const dailyFormHandler = async (event) => {
    ...
    const response = await axios.get(`http://localhost:5000/premium/dailyReport/${day}`, { headers: { 'Authorization': token } });
  ...
  const monthlyFormHandler = async (event) => {
    event.preventDefault();
    const response = await axios.get(`http://localhost:5000/premium/monthlyReport/${month}`, { headers: { 'Authorization': token } });
    const expense = response.data.allExpenses;
    let total = 0;
    for (let obj of expense) {
      total += obj.amount;
  ...
  const downloadDailyReport = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/premium/download/${day}`, {
  ...
  const downloadMonthlyReport = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/premium/download/${month}`, {
        headers: { 'Authorization': token },
      });
      const fileURL = response.data.fileUrl;
      const link = document.createElement('a');
  ...
  const recentlyDownloadedReport = async () => {
    try {
      const response = await axios.get('http://localhost:5000/premium/get-downlodedFileUrls', {
        headers: { 'Authorization': token },
      });
